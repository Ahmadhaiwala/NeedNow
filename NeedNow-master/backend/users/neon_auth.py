...
class NeonAuthValidator:
    ...
    def validate_token(self, token):
        ...
        """
        try:
            # 1. Try decoding with Django SECRET_KEY (HS256)
            try:
                payload = jwt.decode(
                    token,
                    settings.SECRET_KEY,
                ...
                if payload and ('email' in payload or 'sub' in payload or 'id' in payload):
                    return payload
            except Exception:
                pass

            # 2. Decode header to get key ID for JWKS
            header = jwt.get_unverified_header(token)
            kid = header.get("kid")
            alg = header.get("alg")
            
            if not kid:
                # Fallback to payload extraction if token expiry is valid
                try:
            ...
            algorithms = [alg] if alg else ["EdDSA", "RS256", "ES256", "HS256"]
            payload = jwt.decode(
                token,
                public_key,
