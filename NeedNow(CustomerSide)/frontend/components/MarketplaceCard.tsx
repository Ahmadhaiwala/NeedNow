...
export default function MarketplaceCard({
  ...
  const imageUrl = React.useMemo(() => {
    ...
    // Fallback lifestyle placeholder based on category
  ...
  const ownerName = post.owner_details?.first_name 
    ...
    : (post.owner_details?.email?.split('@')[0] || 'Community Member');

  const rating = post.owner_details?.rating || 4.8;
  const reviewCount = post.owner_details?.review_count || 12;
  ...
  const typeBadgeStyle = React.useMemo(() => {
    switch (post.post_type) {
      ...
      case 'donate':
        return { bg: 'rgba(90, 122, 94, 0.12)', color: 'var(--success)', label: 'Free' };
      default:
  ...
  return (
    <motion.div
      ...
      className="group rounded-2xl overflow-hidden cursor-pointer flex flex-col justify-between"
      style={{
    ...
    >
      ...
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-black/5">
        ...
        <div
          ...
          style={{ background: 'rgba(33, 26, 22, 0.75)', color: '#FFF9EF' }}
        >
        ...
        <div>
          ...
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-xs font-semibold truncate max-w-[110px]" style={{ color: 'var(--foreground)' }}>
                {ownerName}
              </span>
            </div>
