...
import WorkspaceDashboard from '@/components/marketplace/WorkspaceDashboard';
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        ...
        {activeMainTab === 'feed' && (
          <div>
            ...
            <ListingCarousel
              title="What your neighborhood is offering"
              subtitle="Neibhours market place"
        ...
        {activeMainTab === 'workspace' && user && (
          <WorkspaceDashboard
            profile={profile}
            savedPostIds={savedPostIds}
            savedPosts={feedData.results.filter((p) => savedPostIds.includes(p.id))}
            onOpenCreate={() => setIsCreateModalOpen(true)}
            onOpenOffers={() => setIsOffersModalOpen(true)}
            onOpenChat={(userId, userName, postId, postTitle) => {
              handleOpenChatForPost(userId, userName, postId, postTitle);
            }}
            onSelectPost={handleSelectPost}
          />
        )}
